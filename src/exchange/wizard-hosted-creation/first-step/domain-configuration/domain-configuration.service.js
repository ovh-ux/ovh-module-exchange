angular
    ...
    .service("wizardHostedCreationDomainConfiguration", class wizardHostedCreationDomainConfiguration {
        ...
        polling (organizationName, exchangeService, task) {
            ...
            return this.Poller
                .poll(`/email/exchange/${organizationName}/service/${exchangeService}/task/${taskId}`);
        }
        ...
        retrievingTask (organizationName, exchangeService, task) {
            ...
            return this.OvhHttp
                .get(`/email/exchange/${organizationName}/service/${exchangeService}/task/${taskId}`, {
                    rootPath: "apiv6"
                });
        }
        ...
        retrievingDomain (organizationName, exchangeService, domainName) {
            ...
            return this.OvhHttp
                .get(`/email/exchange/${organizationName}/service/${exchangeService}/domain/${asciiDomainName}`, {
                    rootPath: "apiv6"
                });
        }

        pollingCNAMEToCheck (organizationName, exchangeService, domainName, associatedTask, deferred = null) {
            ...
            if (associatedTask.status === "todo") {
                this.retrievingDomain(organizationName, exchangeService, domainName)
                    .then((domain) => {
                        ...
                        return this.OvhHttp
                            .get(`/email/exchange/${organizationName}/service/${exchangeService}/task/${domain.taskPendingId}`, {
                                rootPath: "apiv6"
                            });
                    })
                    .then((task) => {
                        if (!_(task).isEmpty()) {
                            this.$timeout(() => this.pollingCNAMEToCheck(organizationName, exchangeService, domainName, task, localDeferred), 5000);
                        }
                    });
            } else {
                this.retrievingDomain(organizationName, exchangeService, domainName)
