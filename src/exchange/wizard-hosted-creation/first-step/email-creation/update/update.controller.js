    ...
    .controller("exchangeWizardHostedCreationEmailCreationUpdateController", class ExchangeWizardHostedCreationEmailCreationUpdateController {
        ...
        formCanBeSubmitted () {
            ...
            if (passwordMustBeChanged) {
                const passwordChangeIsValid = this.addForm.password.$dirty && this.addForm.passwordConfirmation.$dirty;

                return formIsValid && passwordChangeIsValid;
            }

            const formHasChanged = _(this.initialModel).reduce((result, value, key) => value === this.model[key] ? result : result.concat(key), []).length > 0;
        ...
        updatingEmailAddress () {
            ...
            return this.wizardHostedCreationEmailCreation
                .addingOrUpdatingEmailAccount(this.$routerParams.organization, this.$routerParams.productId, this.formerPrimaryEmailAddress, model)
                .then(() => {
                    if (!_(this.model.password).isEmpty()) {
                        return this.wizardHostedCreationEmailCreation.updatingPassword(this.$routerParams.organization, this.$routerParams.productId, `${this.model.login}@${this.model.domain}`, this.model.password);
                    }
