angular
    ...
    .service("wizardHostedCreationEmailCreation", class wizardHostedCreationEmailCreation {
        ...
        checkingMigration (domainName, accountName, destinationServiceName, destinationEmailAddress) {
            ...
            return this.OvhHttp.get(`/email/domain/${asciiDomainName}/account/${accountName}/migrate/${destinationServiceName}/destinationEmailAddress/${destinationEmailAddress}/checkMigrate`, {
                rootPath: "apiv6"
            });
        }

        executingMigration (domainName, accountName, destinationServiceName, destinationEmailAddress, password) {
            const asciiDomainName = punycode.toASCII(domainName);

            return this.OvhHttp.post(`/email/domain/${asciiDomainName}/account/${accountName}/migrate/${destinationServiceName}/destinationEmailAddress/${destinationEmailAddress}/migrate`, {
                rootPath: "apiv6",
                data: {
                    password
                }
        ...
        updatingPassword (organizationName, exchangeService, primaryEmailAddress, password) {
            return this.OvhHttp.post(`/email/exchange/${organizationName}/service/${exchangeService}/account/${primaryEmailAddress}/changePassword`, {
                rootPath: "apiv6",
                data: {
                    password
                }
        ...
        retrievingAvailableAccounts (organizationName, exchangeService) {
            return this.OvhHttp
                ...
                .then((availableAccounts) => {
                    const promises = _(availableAccounts).map((availableAccount) => this.OvhHttp
                        .get(`/email/exchange/${organizationName}/service/${exchangeService}/account/${availableAccount}`, {
                            rootPath: "apiv6"
                        })
                    ).value();
