    ...
    .controller("ExchangeTabDiagnosticsCtrl", class ExchangeTabDiagnosticsCtrl {
        ...
        continueDiagnosticIfAny (email) {
            ...
            this.services
                .diagnostic
                .gettingTasks(email)
                .then((taskIds) => this.services.$q.all(_.map(taskIds, (taskId) => this.services.diagnostic.gettingTask(email, taskId))))
                .then((tasks) => {
        ...
        getDiagnosticResultAndDisplayIt () {
            this.gettingDiagnosticResult()
                .then(() => this.displayDiagnosticResult());
        }
        ...
        makingSupportTicketBody () {
            const diagnosticUrl = URI.expand("/email/exchange/{organizationName}/service/{exchangeService}/account/{primaryEmailAddress}/diagnostics", {
...
${diagnosticJson}`;
        ...
        fetchDiagnosticGuideUrl () {
            ...
            this.services
                .User
                .getUser()
                .then((data) => { this.diagnosticGuideUrl = this.services.EXCHANGE_CONFIG.URLS.GUIDES.DIAGNOSTIC[data.ovhSubsidiary] || this.services.EXCHANGE_CONFIG.URLS.GUIDES.DIAGNOSTIC[defaultSubsidiary]; })
                .catch(() => { this.diagnosticGuideUrl = null; });
        }
    });
