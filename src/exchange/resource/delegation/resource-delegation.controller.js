angular.module('Module.exchange.controllers').controller(
  'ExchangeResourceDelegationCtrl',
  class ExchangeResourceDelegationCtrl {
    ...
    rollChanges(account) {
      ...
      if (buffer) {
        _.set(account, 'newDelegationValue', buffer.newDelegationValue);
      }
    }
    ...
    checkboxStateChange(state) {
      ...
      if (_.has(this.delegationList, 'list.results')) {
        this.delegationList.list.results.forEach((account) => {
          _.set(account, 'newDelegationValue', newDelegationValue);
    ...
    retrieveDelegationRights(count, offset) {
      ...
      return this.services.ExchangeResources.getAccountsByResource(
        ...
        .then((accounts) => {
          ...
          if (_.has(this.delegationList, 'list.results')) {
            this.delegationList.list.results.forEach((account) => {
              _.set(account, 'newDelegationValue', account.delegation);
