angular
    ...
    .controller("ExchangeResourceDelegationCtrl", class ExchangeResourceDelegationCtrl {
        constructor ($scope, Exchange, ExchangeResources, $timeout, messaging, navigation, $translate) {
            ...
            this.debouncedRetrieveDelegationRights = _.debounce(this.retrieveDelegationRights, 300);
            $scope.retrieveDelegationRights = (count, offset) => this.retrieveDelegationRights(count, offset);
        ...
        rollChanges (account) {
            ...
            if (buffer) {
                account.newDelegationValue = buffer.newDelegationValue;
            }
        }

        checkboxStateChange (state) {
            const newDelegationValue = state !== 0;
            this.buffer.selected = newDelegationValue ? _.clone(this.buffer.ids, true) : [];

            if (_.has(this.delegationList, "list.results")) {
                this.delegationList.list.results.forEach((account) => {
                    account.newDelegationValue = newDelegationValue;
        ...
        hasChangedClass (id) {
            return _.find(this.buffer.changes, { id }) ? "font-weight-bold text-info" : null;
        }

        hasChanged () {
            return this.buffer.changes != null && !_.isEmpty(this.buffer.changes);
        }
        ...
        retrieveDelegationRights (count, offset) {
            ...
            return this.services
                .ExchangeResources
                .getAccountsByResource(this.$routerParams.organization, this.$routerParams.productId, this.resource.resourceEmailAddress, count, offset, this.form.search)
                .then((accounts) => {
                    this.buffer.selected = [];
                    this.buffer.ids = _.map(accounts.list.results, "id");

                    this.delegationList = accounts; // make a deep copy of accounts list to use it as model

                    if (_.has(this.delegationList, "list.results")) {
                        this.delegationList.list.results.forEach((account) => {
                            account.newDelegationValue = account.delegation;
