    ...
    .service("exchangeHeaderLicence", class ExchangeHeaderLicence {
        ...
        fetchLicences (organizationName, exchangeService, startingDate) {
            return this.OvhHttp
                ...
                .then((licenses) => ({
                    maxValue: _.max(_(licenses).map("outlookQuantity").value()
                        .concat(_(licenses).map((license) => _(license.accountLicense).filter({ license: "standard" }).value()[0].licenseQuantity).value())
                        .concat(_(licenses).map((license) => _(license.accountLicense).filter({ license: "basic" }).value()[0].licenseQuantity).value())
                        .concat(_(licenses).map((license) => _(license.accountLicense).filter({ license: "enterprise" }).value()[0].licenseQuantity).value())),
                    outlook: _(licenses).map((license) => ({
                        x: ExchangeHeaderLicence.getDate(license.date),
                        y: license.outlookQuantity
                    })).value(),
                    standard: _(licenses).map((license) => ({
                        x: ExchangeHeaderLicence.getDate(license.date),
                        y: _(license.accountLicense).filter({ license: "standard" }).value()[0].licenseQuantity
                    })).value(),
                    basic: _(licenses).map((license) => ({
                        x: ExchangeHeaderLicence.getDate(license.date),
                        y: _(license.accountLicense).filter({ license: "basic" }).value()[0].licenseQuantity
                    })).value(),
                    enterprise: _(licenses).map((license) => ({
                        x: ExchangeHeaderLicence.getDate(license.date),
                        y: _(license.accountLicense).filter({ license: "enterprise" }).value()[0].licenseQuantity
                    })).value()
                }))
