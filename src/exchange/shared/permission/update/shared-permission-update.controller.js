    ...
    .controller("ExchangeUpdatePublicFolderPermissionCtrl", class ExchangeUpdatePublicFolderPermissionCtrl {
        ...
        retrievingPermissions (count, offset) {
            ...
            return this.services
                .ExchangePublicFolders
                .retrievingAccountsByPublicFolder(this.$routerParams.organization, this.$routerParams.productId, this.folder.path, count, offset, this.searchValue)
                .then((data) => {
        ...
        mapPermissionToArray () {
            if (!_.has(this.permissions, "current.list.results") || this.permissions.current.list.results == null) {
                throw "mapPermissionToArray() can only be used once the permission retrieved from the server";
            }

            this.permissions.selectedPermissions = _.mapValues(this.permissions.selectedPermissions, () => []);
            this.permissions.ids = this.permissions.current.list.results.map((permission) => permission.accessRights);
            this.permissions.former.list.results.forEach((permission) => {
                const account = _.keys(this.permissions.changes).find((accountName) => accountName === permission.primaryAddressDisplayName);

                if (account == null) {
                    this.changeAccountAndPermission(permission.primaryAddressDisplayName, permission.accessRights);
                } else {
                    this.changeAccountAndPermission(permission.primaryAddressDisplayName, this.permissions.changes[account]);
                }
            });
        }
        ...
        updateAccountPermission (accountName, permission) {
            ...
            const matchingAccount = this.permissions.current.list.results.find((currentAccount) => currentAccount.primaryAddressDisplayName === accountName);
        ...
        deletePreviousPermissionEntry (accountName) {
            ...
            if (!_.isEmpty(previousPermissionName)) {
                this.permissions.selectedPermissions[previousPermissionName] = this.permissions.selectedPermissions[previousPermissionName].filter((currentAccountName) => currentAccountName !== accountName);
            }
        ...
        getOperationType (accountName) {
            ...
            const current = this.permissions.current.list.results.find((formerPermission) => formerPermission.primaryAddressDisplayName === accountName);
